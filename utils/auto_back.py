...
_BACK_RULES: List[Tuple[str, Callable[[re.Match[str]], str]]] = [
    ...
    (r"^admin:grant_access_(\d+)$", lambda m: f"admin:show_user_subscriptions_page_{m.group(1)}_0"),
    (r"^admin:show_subscription_(\d+)_(\d+)$", lambda m: f"admin:show_user_subscriptions_page_{m.group(1)}_0"),
    ...
    # Admin: Tickets
    (r"^admin:tickets_menu$", lambda m: f"admin:back_to_menu"),
    (r"^admin:ticket_(\d+)$", lambda m: f"admin:tickets_menu_page_{m.group(1)}"),
