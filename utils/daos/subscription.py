...
class SubscriptionDAO:
    ...
    @staticmethod
    async def update_status(session: AsyncSession, subscription_id: int, new_status: SubscriptionStatus) -> SubscriptionReadSchemaDB | None:
        result = await session.execute(
    ...
    @staticmethod
    async def update_access_period(session: AsyncSession, subscription_id: int, access_from: datetime, access_to: datetime) -> SubscriptionReadSchemaDB | None:
        result = await session.execute(
            select(Subscription).where(Subscription.id == subscription_id)
        )
        subscription: Subscription | None = result.scalars().first()
    ...
    @staticmethod
    async def get_active_subscriptions_by_user_id(session: AsyncSession, user_id: int) -> List[SubscriptionReadSchemaDB]:
        result = await session.execute(
    ...
    @staticmethod
    async def update_subscription_user_id_by_subscription_id(session: AsyncSession, subscription_id: int, user_id: int) -> SubscriptionReadSchemaDB | None:
        result = await session.execute(
            select(Subscription).where(Subscription.id == subscription_id)
        )
        subscription: Subscription | None = result.scalars().first()
