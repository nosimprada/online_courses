...
async def update_subscription_status(subscription_id: int, new_status: SubscriptionStatus) -> SubscriptionReadSchemaDB | None:
    async with AsyncSessionLocal() as session:
        return await SubscriptionDAO.update_status(session, subscription_id, new_status)
    
async def update_subscription_access_period(subscription_id: int, access_from: datetime.datetime, access_to: datetime.datetime) -> SubscriptionReadSchemaDB | None:
    async with AsyncSessionLocal() as session:
...
async def update_subscription_user_id_by_subscription_id(subscription_id: int, user_id: int) -> SubscriptionReadSchemaDB | None:
    async with AsyncSessionLocal() as session:
